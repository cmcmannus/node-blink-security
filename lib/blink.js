      ...
      .then(results => {
        return results.reduce((acc, result, index) => {
          ...
          this._networks.forEach(network => {
            if (network.id !== networks[index]) return;
            network = {
              ...network,
              ...result,
            };
    ...
    networks.forEach(networkId => {
      promises.push(new Promise((resolve, reject) => {
        request({
          ...
          if (err || _statusCodeIsError(response)) {
            return reject(new BlinkException(`Can't retrieve system events`));
          } 
          this._events = body.event;
          return resolve(this._events);
        });
      }));
    });
    ...
    networks.forEach(networkId => {
      promises.push(new Promise((resolve, reject) => {
        request({
          ...
          return resolve(body.syncmodule.status === 'online');
        });
      }));
    });
  ...
  getClients() {
    return new Promise((resolve, reject) => {
      request({
        ...
        if (err || _statusCodeIsError(response)) {
          return reject(new BlinkException(`Can't retrieve connected clients`));
        }
        return resolve(body);
      });
    });
  ...
  _getAuthToken(repeats = 0) {
    return new Promise((resolve, reject) => {
      ...
      let headers = {
        ...
        'Host': DEFAULT_URL,
        'User-Agent': 'ColeHomeAutomation Client'
      };
      let data = {
      ...
      let authenticate = (response) => {
        ...
        this._client_id = response.auth.client_id;
        this._account_id = response.auth.account_id;
      ...
      request.post({
        ...
        if (err || _statusCodeIsError(response)) {
          ...
          if (!body.account?.tier) {
            return reject(new BlinkAuthenticationException(body));
          }
      ...
      request({
        ...
        if (err || _statusCodeIsError(response)) {
          ...
          if (typeof name_or_id != 'undefined') {
            body.summary.forEach(function(n){
